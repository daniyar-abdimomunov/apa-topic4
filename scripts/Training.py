...
# Forecasting setup
LOOKBACK = 168 #number of historical hours per sample
HORIZON = 24 #number of steps to predict ahead
...
train_true = torch.Tensor(train_true[:, : , GERMANY_INDEX]) # we only want to make predictions for one of the countries, so we select the column with the relevant keys as our true values
test_input = torch.Tensor(test_input)
test_true = torch.Tensor(test_true[:, : , GERMANY_INDEX]) # we only want to make predictions for one of the countries, so we select the column with the relevant keys as our true values
...
# %%
train_input_univariate = train_input[:, :, GERMANY_INDEX]
train_input_univariate_os = scaler.inverse_transform(train_input_univariate)
test_input_univariate = test_input[:, :, GERMANY_INDEX]
test_input_univariate_os = scaler.inverse_transform(test_input_univariate)
test_input_univariate.shape

# %%
# save test data
np.save('../data/inputs_univariate.npy', test_input_univariate_os)
...
# %%
# Fit and predict
ts_mixer.fit(train_loader, device='cpu', epochs=50, lr=1e-3)
...
# %%
# Train and predict
patch_TST.fit(train_loader_patch, device='cpu', epochs=100, lr=1e-3)
...
train_loader_svgp = DataLoader(train_dataset, batch_size=16,
                               shuffle=True)  # batch_size (M), batch shape = ([M, L, V0], [M, H, V1])
...
# %%
neural_tsgp.fit(train_loader_svgp, num_data=train_input.size(0), epochs=100)
...
patch_tst_gp = PatchTSTGPModel(inducing_points, HORIZON, LOOKBACK, num_latents_svgp=NUM_LATENTS_SVGP, num_layers=6,
                               patch_size=21, embed_dim=8)

# %%
patch_tst_gp.fit(train_loader_svgp, num_data=train_input.size(0), epochs=100)
...
ts_mixer_gp = TSMixerGPModel(inducing_points, horizon=HORIZON, lookback=LOOKBACK, num_latents_svgp=NUM_LATENTS_SVGP,
                             num_latents_lfe=NUM_LATENTS_LFE)
# %%
ts_mixer_gp.fit(train_loader_svgp, num_data=train_input.size(0), epochs=100)
...
# %% [markdown]
# ## 4. Univariate GP Models Training

# %%
# Setup for SVGP models using univariate data: inducing points , latents , and a smaller batch size
train_dataset_uni = TensorDataset(train_input_univariate.unsqueeze(-1), train_true)
train_loader_svgp_uni = DataLoader(train_dataset_uni, batch_size=16,
                                   shuffle=True)  # batch_size (M), batch shape = ([M, L, V0], [M, H, V1])

# %%
NUM_INDUCING_POINTS = 200  # num_inducing_points (P)
NUM_LATENTS_SVGP = 8  # num_latents_svgp (lf0)
inducing_points_uni = train_input_univariate.unsqueeze(-1)[:NUM_INDUCING_POINTS]

# %% [markdown]
# ### 4.1 Base Multistep SVGP (Univariate)

# %%
# A sparse variational GP that maps from the sequence inputs to Horizon outputs
tsgp_model_uni = TSGPModel(inducing_points_uni, HORIZON, num_latents_svgp=NUM_LATENTS_SVGP)

# %%
# Train GP model
tsgp_model_uni.fit(train_loader_svgp_uni, num_data=train_input_univariate.size(0), epochs=100)

# %%
# Inference with TSGP in scaled space with means /lower / upper
tsgp_uni_preds, tsgp_uni_lowers, tsgp_uni_uppers = tsgp_model_uni.predict(test_input_univariate)

# %%
# separate predictions and confidence interval
# Inverse transform to original scale for saving and plotting
tsgp_uni_preds = scaler.inverse_transform(tsgp_uni_preds.detach().numpy())
tsgp_uni_lowers = scaler.inverse_transform(tsgp_uni_lowers.detach().numpy())
tsgp_uni_uppers = scaler.inverse_transform(tsgp_uni_uppers.detach().numpy())


# %%
# export forecast outputs for benchmarking
np.save(os.path.join(DATA_DIR, 'tsgp_uni_preds.npy'), tsgp_uni_preds)
np.save(os.path.join(DATA_DIR, 'tsgp_uni_lowers.npy'), tsgp_uni_lowers)
np.save(os.path.join(DATA_DIR, 'tsgp_uni_uppers.npy'), tsgp_uni_uppers)
...
plot_predictions(
    test_input_univariate_os[TEST_CASE],
    test_true_os[TEST_CASE],
    tsgp_uni_preds[TEST_CASE],
    tsgp_uni_lowers[TEST_CASE],
    tsgp_uni_uppers[TEST_CASE],
    title=f'TSGP Model Predictions\nTest Case: #{TEST_CASE}'
)

# %% [markdown]
# ### 4.2 Neural SVGP (Univariate)

# %%
# Adds a deep feature extractor before the GP layer for richer representations
NUM_LATENTS_LFE = 8
neural_tsgp_uni = NeuralTSGPModel(inducing_points_uni, HORIZON, num_latents_svgp=NUM_LATENTS_SVGP,
                                  num_latents_lfe=NUM_LATENTS_LFE)

# %%
neural_tsgp_uni.fit(train_loader_svgp_uni, num_data=train_input_univariate.size(0), epochs=35)

# %%
# Inference with Neural TSGP
neural_tsgp_uni_preds, neural_tsgp_uni_lowers, neural_tsgp_uni_uppers = neural_tsgp_uni.predict(test_input_univariate.unsqueeze(-1), true_shape=test_true.shape)

# %%
# separate predictions and confidence interval
neural_tsgp_uni_preds = scaler.inverse_transform(neural_tsgp_uni_preds.detach().numpy())
neural_tsgp_uni_lowers = scaler.inverse_transform(neural_tsgp_uni_lowers.detach().numpy())
neural_tsgp_uni_uppers = scaler.inverse_transform(neural_tsgp_uni_uppers.detach().numpy())


# %%
# export
np.save(os.path.join(DATA_DIR, 'neural_tsgp_uni_preds.npy'), neural_tsgp_uni_preds)
np.save(os.path.join(DATA_DIR, 'neural_tsgp_uni_lowers.npy'), neural_tsgp_uni_lowers)
np.save(os.path.join(DATA_DIR, 'neural_tsgp_uni_uppers.npy'), neural_tsgp_uni_uppers)
...
plot_predictions(
    test_input_univariate_os[TEST_CASE],
    test_true_os[TEST_CASE],
    neural_tsgp_uni_preds[TEST_CASE],
    neural_tsgp_uni_lowers[TEST_CASE],
    neural_tsgp_uni_uppers[TEST_CASE],
    title=f'Neural TSGP Model Predictions\nTest Case: #{TEST_CASE}'
)

# %% [markdown]
# ### 4.3 PatchTST Extension for Time-series GP (Univariate)

# %%
patch_tst_gp_uni = PatchTSTGPModel(inducing_points_uni, HORIZON, LOOKBACK, num_latents_svgp=NUM_LATENTS_SVGP, num_layers=6,
                     patch_size=21, embed_dim=8)

# %%
patch_tst_gp_uni.fit(train_loader_svgp_uni, num_data=train_input.size(0), epochs=100)

# %%
# Inference with PatchTST GP
patch_tst_gp_uni_preds, patch_tst_gp_uni_lowers, patch_tst_gp_uni_uppers = patch_tst_gp_uni.predict(test_input_univariate.unsqueeze(-1), true_shape=test_true.shape)

# %%
# separate predictions and confidence interval
patch_tst_gp_uni_preds = scaler.inverse_transform(patch_tst_gp_uni_preds.detach().numpy())
patch_tst_gp_uni_lowers = scaler.inverse_transform(patch_tst_gp_uni_lowers.detach().numpy())
patch_tst_gp_uni_uppers = scaler.inverse_transform(patch_tst_gp_uni_uppers.detach().numpy())

# %%
# export
np.save(os.path.join(DATA_DIR, 'patch_tst_gp_uni_preds.npy'), patch_tst_gp_uni_preds)
np.save(os.path.join(DATA_DIR, 'patch_tst_gp_uni_lowers.npy'), patch_tst_gp_uni_lowers)
np.save(os.path.join(DATA_DIR, 'patch_tst_gp_uni_uppers.npy'), patch_tst_gp_uni_uppers)
...
plot_predictions(
    test_input_univariate_os[TEST_CASE],
    test_true_os[TEST_CASE],
    patch_tst_gp_uni_preds[TEST_CASE],
    patch_tst_gp_uni_lowers[TEST_CASE],
    patch_tst_gp_uni_uppers[TEST_CASE],
    title=f'PatchTST Neural GP Model Predictions\nTest Case: #{TEST_CASE}'
)

# %% [markdown]
# ### 4.4 TSMixer Extension for Time-series GP (Univariate)

# %%
# Uses TSMixer encoder with a GP head for probabilistic outputs
ts_mixer_gp_uni = TSMixerGPModel(inducing_points_uni, horizon=HORIZON, lookback=LOOKBACK, num_latents_svgp=NUM_LATENTS_SVGP,
                             num_latents_lfe=NUM_LATENTS_LFE)

# %%
ts_mixer_gp_uni.fit(train_loader_svgp_uni, num_data=train_input_univariate.size(0), epochs=100)

# %%
# Inference with TSMixer GP
ts_mixer_gp_uni_preds, ts_mixer_gp_uni_lowers, ts_mixer_gp_uni_uppers = ts_mixer_gp_uni.predict(test_input_univariate.unsqueeze(-1), true_shape=test_true.shape)

# %%
# separate predictions and confidence interval
ts_mixer_gp_uni_preds = scaler.inverse_transform(ts_mixer_gp_uni_preds.detach().numpy())
ts_mixer_gp_uni_lowers = scaler.inverse_transform(ts_mixer_gp_uni_lowers.detach().numpy())
ts_mixer_gp_uni_uppers = scaler.inverse_transform(ts_mixer_gp_uni_uppers.detach().numpy())

# %%
# export
np.save(os.path.join(DATA_DIR, 'ts_mixer_gp_uni_preds.npy'), ts_mixer_gp_uni_preds)
np.save(os.path.join(DATA_DIR, 'ts_mixer_gp_uni_lowers.npy'), ts_mixer_gp_uni_lowers)
np.save(os.path.join(DATA_DIR, 'ts_mixer_gp_uni_uppers.npy'), ts_mixer_gp_uni_uppers)
...
plot_predictions(
    test_input_univariate_os[TEST_CASE],
    test_true_os[TEST_CASE],
    ts_mixer_gp_uni_preds[TEST_CASE],
    ts_mixer_gp_uni_lowers[TEST_CASE],
    ts_mixer_gp_uni_uppers[TEST_CASE],
    title=f'TS Mixer NeuralGP Model Predictions\nTest Case: #{TEST_CASE}'
)
